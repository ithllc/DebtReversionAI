...
async def get_stock_data(ticker: str, period: str = "1y") -> str:
    """
    Get stock price data including 52-week high/low
    ...
    Returns:
        Stock data summary with price, 52-week range, and distance from high/low
    """
...
async def calculate_macd(ticker: str, timeframe: str) -> str:
    """
    Calculate MACD indicator for daily and weekly timeframes
    ...
    Returns:
        MACD analysis with line, signal, histogram, and trend indication
    """
...
async def check_52week_low(ticker: str, tolerance: float = 0.05) -> str:
    """
    Check if stock is at or near 52-week low
    
    Args:
        ticker: Stock ticker symbol
        tolerance: Percentage tolerance from low (default: 0.05 = 5%)
    
    Returns:
        Analysis of proximity to 52-week low
    """
...
async def check_optionable(ticker: str) -> str:
    """
    Verify if stock has options available
    
    Args:
        ticker: Stock ticker symbol
    
    Returns:
        Whether options are available and expiration details
    """
...
async def search_debt_conversions(ticker: str, months_back: int = 3) -> str:
    """
    Search for debt conversion events in 8-K filings
    
    Args:
        ticker: Stock ticker symbol
        months_back: Number of months to search back (default: 3)
    
    Returns:
        Summary of debt conversion events found in SEC filings
    """
...
async def get_recent_filings(ticker: str, form_type: str = "8-K", count: int = 10) -> str:
    ...
    Args:
        ticker: Stock ticker symbol
        form_type: Type of SEC form (default: 8-K)
        count: Number of filings to retrieve (default: 10)
    
    Returns:
        List of recent SEC filings with dates and URLs
    """
